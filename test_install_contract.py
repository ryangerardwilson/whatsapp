...
class InstallContractTests(unittest.TestCase):
    ...
    def test_dash_v_without_argument_prints_latest_release(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_executable(
                ...
                "  printf 'https://github.com/ryangerardwilson/whatsapp/releases/tag/v0.1.21\\n'\n"
                "  exit 0\n"
                "fi\n"
    ...
    def test_upgrade_same_version_uses_dash_v(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((Path('$HOME/.local/bin'.replace("$HOME", str(home_dir))) / 'whatsapp').exists())
    ...
    def test_local_binary_install_writes_managed_launchers(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            internal_launcher = home_dir / ".whatsapp" / "bin" / 'whatsapp'
            self.assertTrue(internal_launcher.exists())
            self.assertEqual(
                bashrc_path.read_text(encoding='utf-8'),
                '# existing shell config\n',
            )
            public_launcher = Path('$HOME/.local/bin'.replace("$HOME", str(home_dir))) / 'whatsapp'
